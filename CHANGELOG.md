...
- **CodeBuddy Code support** - the plugin also installs as a CodeBuddy Code plugin: `.codebuddy-plugin/plugin.json` (manifest resolved in the order `.codebuddy-plugin` → `.workbuddy-plugin` → `.claude-plugin`) registers `hooks/hooks-codebuddy.json`, which maps `PreToolUse`, `Notification`, `SessionStart`, `Stop`, `SubagentStop`, and `TeammateIdle` onto the same `handle-hook` pipeline. CodeBuddy Code is hook-compatible with Claude Code (identical events and stdin payload) and also exports `CLAUDE_*` aliases, so it is identified by its `CODEBUDDY_*` environment variables; `product.Detect` checks them before the `PLUGIN_ROOT`/Codex heuristic so a leaked `PLUGIN_ROOT` cannot shadow CodeBuddy. The hook commands use the single-string form (`sh "…/hook-wrapper.sh" handle-hook <Event> --product codebuddy`) because CodeBuddy ignores the `args` array and has no `command_windows` support (Git Bash is forced on Windows). On `Stop`/`SubagentStop` the handler parses the CodeBuddy `transcript_path` through the new `jsonl.ParseCodeBuddyFile` normalizer (which maps `type:"message"`+`role`, `output_text`/`function_call`, and epoch-millisecond timestamps into the shared `jsonl.Message` shape, dropping injected/`skipRun`/`isMeta` scaffolding, and coalescing orphan tool calls) and falls back to `last_assistant_message` classification when no transcript is present. `EnterPlanMode` is now a no-op PreToolUse status, `PowerShell` is treated as an active tool, and `Agent`/`Skill`/`TaskGet`/`TaskList`/`ToolSearch`/`SendMessage`/`DeferExecuteTool` as passive. Configuration is additionally read from `~/.codebuddy/agent-notifications-go/config.json`.
