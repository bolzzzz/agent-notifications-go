#!/bin/bash
# bootstrap.sh - One-command install/update for agent-notifications plugin
# Usage: curl -fsSL https://raw.githubusercontent.com/777genius/agent-notifications-go/main/bin/bootstrap.sh | bash
...
install_plugin() {
    ...
    local stable_config_dir="${CLAUDE_HOME}/agent-notifications-go"
    if [ -d "$version_dir" ]; then
...
#!/bin/sh
# agent-notifications-go shim: forwards old cached hook path to current plugin installPath.
...
INSTALLED_JSON="${CLAUDE_HOME}/plugins/installed_plugins.json"
MARKETPLACE_NAME="agent-notifications-go"
PLUGIN_KEY="agent-notifications-go@agent-notifications-go"
PLUGIN_ROOT=""
...
if [ -z "$PLUGIN_ROOT" ]; then
  ...
  _parent="$(cd "$_ver_root/.." 2>/dev/null && pwd)"       # .../agent-notifications-go/<versions>
  for d in "$_parent"/*/; do
    [ -d "$d" ] || continue
...
if [ -z "$PLUGIN_ROOT" ]; then
  _PTR_FILE="${CLAUDE_HOME}/agent-notifications-go/plugin-root"
  if [ -f "$_PTR_FILE" ]; then
...
find_plugin_root() {
    ...
    if [ -z "$PLUGIN_ROOT" ]; then
        # Find the installPath that's inside the agent-notifications-go cache dir
...
print_success() {
    ...
    echo -e "  2. Run ${BOLD}/agent-notifications-go:settings${NC} to configure sounds"
